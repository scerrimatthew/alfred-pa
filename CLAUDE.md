...
## Claude model policy (cost management)

Models are matched to task difficulty — premium models only where judgment quality is user-visible. When adding a Claude call, pick from this ladder rather than defaulting to Opus:

| Task | Model | Why |
|---|---|---|
| Personal email triage | Haiku 4.5 | High volume (every inbox email), rubric-driven classification |
| `/joke` | Haiku 4.5 | It's a joke |
| School email summarize | Sonnet 4.5 | Date math on weekly plans; errors mean missed school events |
| Evening/personal digests, school chat Q&A | Sonnet 4.6 | Reformatting already-summarized data / answering from provided context |
| Personal chat Q&A (tool loop) | Sonnet 4.6 | Tool orchestration; system+tools prefix is prompt-cached across loop iterations |
| AI-news midday flash check | Sonnet 5 | Runs daily, almost always "nothing urgent"; a miss waits for the evening digest |
| Weekly ETF report | Sonnet 5 | Factual price read with light narrative |
| AI-news evening digest + weekly synthesis | Opus 5 | The watchlist-relevance judgment is the feature |
| `/evolve` coding sessions | Sonnet (default) | Biggest single credit burner; `/evolve opus <instruction>` opts into Opus for big/delicate features |

The two loop-shaped call sites (personal Q&A tool loop, news research `pause_turn` resumes) set a prompt-cache breakpoint on the system prompt (`PromptCacheType.FineGrained` + `CacheControl` ephemeral), so the repeated prefix bills at ~10% on iterations after the first.
...
12. **AI news digest**: **AiNewsDigest** (timer, daily 18:00 UTC / 8 PM CEST trigger, but gated in-code to fire every other day — see below) sends the personal chat an evening AI-news briefing. `ClaudeNewsResearchService` calls Claude (`claude-opus-5`) with the Anthropic **server-side web search tool** (max 12 searches, `pause_turn` resumed up to 5 times); the prompt embeds the Cleverbit watchlist brief (`Services/AI/AiNewsBriefing.cs` — a mirror of "AI News Watchlist — PA Agent Brief" on Matthew's OneDrive; update the class when the source doc is revised). Claude ranks the last ~24-72h of news by relevance to the watchlist (max `AiNewsMaxItems` stories, PA-voice format: linked headline + one-line summary + "why it matters to us"; disconfirming thesis evidence flagged first, TL material tagged). Quiet days send nothing. Reported stories land in the `ReportedNews` table (keyed by URL hash, with the summary/why-it-matters stored for follow-ups; 14-day dedup window fed back into the prompt, pruned after 60 days) so nothing repeats unless the implication changed. Matthew steers coverage from chat — "stop covering funding rounds", "more on EU AI Act" — via `add_news_rule` / `list_news_rules` / `remove_news_rule` tools backed by the `NewsRules` table; active rules are injected into every research prompt and override the brief where they conflict. Disabled via `Alfred__AiNewsEnabled=false` or an empty `PersonalTelegramChatId`. The timer still fires daily, but `AiNewsDigestFunction.IsDigestDay` skips every other run — the day count since a fixed epoch (`AiNewsDigestFunction.DigestCadenceEpoch`), mod 2, decides whether today sends; anchored to an epoch date rather than day-of-year so the cadence doesn't wobble across a year boundary. Around the digest:
    - **Midday flash check**: **AiNewsFlash** (timer, daily 10:00 UTC / noon CEST, so urgency is daily-capped by construction) runs a lightweight sweep (max 6 searches, on `claude-sonnet-5` per the cost policy) for the brief's flag-level cases only — a competitor launching into the A-SDLC niche, an Anthropic partner-program change with a deadline, thesis-level disconfirming evidence, or regulation with a compliance clock. Nearly every run finds nothing and stays silent; a hit sends a "🚨" alert immediately and records the stories in `ReportedNews` so the evening digest doesn't repeat them. Runs every day regardless of the digest's every-other-day cadence — it's the thing that catches anything urgent on the digest's off day. Disabled via `Alfred__AiNewsFlashEnabled=false`
    ...
    - **Newsletter mining**: personal triage extracts `newsLeads` (headline/url/note) from AI-industry newsletters passing through the inbox; the monitor saves them to the `NewsCandidates` table (keyed by URL hash, pruned after 7 days). The evening and on-demand research runs receive the last ~50h of candidates as unverified leads to check (a little over two days, to span the digest's off day) — catching stories web search misses. Suppressed (muted) newsletters are still mined — a mute governs alerts, not mining. Backfill batches don't mine (stale leads)
    - **On-demand /ai-news**: `/ai-news` from the personal DM triggers the research immediately (ignoring the every-other-day gate — it always runs); `/ai-news <topic>` makes it a targeted sweep (topic focus beats watchlist breadth, window may extend past 72h). `/ai_news` (the underscore form, the only one Telegram's command menu can register) works identically; the old `/news` name replies with a pointer to the new one. Sends an "on it" ack, then the briefing — and unlike the timer, a quiet result still gets a reply. A single-row `NewsRequests` marker (10-min timeout) guards against concurrent runs; an `/ai-news` sent while one is in flight gets a "still working on it" reply
...
Sending `/evolve <instruction>` to the bot from the personal DM dispatches the `evolve` GitHub Actions workflow (`.github/workflows/evolve.yml`), which runs a headless Claude Code session against this repo, builds, commits, pushes, deploys to Azure, and reports back via Telegram. Sessions run on **Sonnet by default** (cost policy — see "Claude model policy"); `/evolve opus <instruction>` (or the `model` input when dispatching from the Actions tab) runs the session on Opus for big or delicate features. The session's progress streams live into the Actions log (stream-json rendered by `.github/scripts/evolve-stream-filter.jq`) — feature-sized instructions legitimately take 30-60+ minutes, so a quiet-looking run is usually just working. The coding step times out at 90 minutes (which triggers the failure notification), and cancelling an in-progress run notifies Telegram too (a run cancelled while still queued behind another evolve run stays silent — its job never starts). Requirements:
